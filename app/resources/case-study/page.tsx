  ...
  return (
    <div className="flex flex-col gap-12 pb-16 pt-16">
      ...
      <section className="container mx-auto px-4 sm:px-6 lg:px-8 max-w-4xl">
        <FadeInUp>
          <div className="border-b border-slate-200 pb-8">
            ...
            <p className="text-lg text-slate-500 font-medium">Turning conversation data into better student support.</p>
          </div>
        </FadeInUp>
      </section>
      ...
      <article className="container mx-auto px-4 sm:px-6 lg:px-8 max-w-3xl prose prose-lg prose-headings:text-secondary prose-p:text-slate-600 prose-li:text-slate-600 prose-strong:text-secondary">
        {/* Executive Summary */}
        <FadeInUp delay={0.1}>
          <div className="bg-linear-to-br from-green-50/50 to-emerald-50/50 p-8 rounded-xl border border-green-100/50 not-prose mb-12 shadow-sm transition-scale">
          ...
          <FadeInUp key={example.title} delay={0.3 + i * 0.1}>
            <div className="not-prose my-8 p-6 bg-white rounded-xl border border-slate-100 shadow-sm transition-lift">
              <h3 className="text-xl font-bold text-secondary mb-4 flex items-center gap-2">
                <span className="w-8 h-8 rounded-lg bg-linear-to-br from-green-100 to-emerald-100 flex items-center justify-center text-green-600 text-sm font-bold">
                  {i + 1}
                </span>
                {example.title}
              </h3>
              <div className="space-y-3 text-slate-600">
                <p>
                  <span className="font-semibold text-secondary">Insight:</span> {example.insight}
                </p>
                <p>
                  <span className="font-semibold text-secondary">Action:</span> {example.action}
                </p>
                <p className="text-green-700 font-medium">
                  <span className="font-semibold text-green-800">Result:</span> {example.result}
                </p>
              </div>
            </div>
          </FadeInUp>
        ...
        <FadeInUp delay={0.6}>
          ...
          <ul>
            ...
            <li>
              <strong>Data-Driven</strong> resource allocation for the next academic year.
            </li>
          </ul>
        </FadeInUp>
      ...
      {/* CTA Section */}
      <section className="bg-linear-to-br from-secondary via-slate-800 to-secondary text-white py-16 mt-8 rounded-3xl shadow-2xl mx-4 sm:mx-6 lg:mx-8 overflow-hidden relative">
        ...
        <div className="container mx-auto px-4 sm:px-6 lg:px-8 text-center relative z-10">
          ...
          <FadeInUp delay={0.1}>
            <div className="flex justify-center">
              <Button
                href="/demo"
                className="bg-linear-to-r from-primary to-amber-500 hover:from-primary-dark hover:to-amber-600 text-white border-none shadow-lg"
              >
                Request Demo
              </Button>
            </div>
